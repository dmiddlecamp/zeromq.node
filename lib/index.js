  ...
  try {
    while (true) {
      ...
      if (flags & zmq.ZMQ_POLLIN) {
          ...
          //djm ---
          if (self.poll_without_recv) {
              //emit a 'messageready'
              emitArgs = ['message_ready'];

              //immediately deal with the message ready event, and bail
    ...
    this.emit('error', err);
    throw err;  //rethrow here so we can handle broken routing
  }
  finally
  {
    this._flushing = false;
...
/**
 * -- should poll without recv, should emit an event
 *
 * should allow separate polling
 * should allow separate reading
 *
 */


//Should be called from _flush, allows us to see if we have a message
//without pulling it off the socket
Socket.prototype.receive_poll = function() {
     return (this._ioevents & zmq.ZMQ_POLLIN);
};

Socket.prototype.receive = function() {
    var msgs = [];

    if (this.receive_poll()) {
        do {
            msgs.push(this._zmq.recv());
        } while (this._receiveMore);
    }
    return msgs;
};

Socket.prototype.receiveOne = function() {
    var self = this,
        emitArgs;
